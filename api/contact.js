...
import { analyzeContactSpam, getSpamResponse } from '../server/spamProtection.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const spamCheck = analyzeContactSpam(payload);
    if (!spamCheck.allowed) {
      console.warn('[api/contact] Spam blocked:', spamCheck.reason, { email, subject });
      res.status(200).json(getSpamResponse());
      return;
    }
