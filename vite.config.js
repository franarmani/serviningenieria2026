...
import { analyzeContactSpam } from './server/spamProtection.js'
      ...
      configureServer(server) {
        server.middlewares.use('/api/contact', async (req, res, next) => {
          ...
          try {
            ...
            const spamCheck = analyzeContactSpam(body)
            if (!spamCheck.allowed) {
              console.warn('[vite contact api] Spam blocked:', spamCheck.reason, { email, subject })
              res.statusCode = 200
              res.setHeader('Content-Type', 'application/json')
