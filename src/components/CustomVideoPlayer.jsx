...
const CustomVideoPlayer = ({ 
  ...
  return (
    ...
    >
      ...
      {hasVideoError && (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-gradient-to-br from-[#111] to-black px-6 text-center">
          <div className="max-w-md">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-corporate-red/15 text-corporate-red">
              <svg className="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
              </svg>
            </div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-white/70" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
              {language === 'es' ? 'Video no disponible' : 'Video unavailable'}
            </p>
            <p className="mt-3 text-sm leading-relaxed text-white/55" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
              {language === 'es'
                ? 'El archivo multimedia debe volver a cargarse en el sitio para poder reproducirse.'
                : 'The media file must be uploaded again to the site before it can be played.'}
            </p>
          </div>
        </div>
      )}
