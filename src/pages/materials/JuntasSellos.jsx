...
const JuntasSellos = () => {
  ...
  const tiposJuntas = [
    {
      tipo: language === 'es' ? "Juntas Metálicas" : "Metallic Gaskets",
      descripcion: language === 'es' ? "Sellado metal-metal para alta presión y temperatura" : "Metal-to-metal sealing for high pressure and temperature",
      imagen: "https://images.unsplash.com/photo-1581092451866-a33e4b30a2e6?q=80&w=800&auto=format&fit=crop",
      especificaciones: [language === 'es' ? "Acero inoxidable" : "Stainless steel", "Inconel, Hastelloy", "API 6A", "ASME B16.20"],
      aplicaciones: [language === 'es' ? "Alta presión" : "High pressure", language === 'es' ? "Alta temperatura" : "High temperature", language === 'es' ? "Servicios críticos" : "Critical services", "Oil & Gas"]
    },
    {
      tipo: language === 'es' ? "Juntas RTJ" : "RTJ Gaskets", 
      descripcion: language === 'es' ? "Ring Type Joint para conexiones bridadas API" : "Ring Type Joint for API flanged connections",
      imagen: "https://images.unsplash.com/photo-1565088299075-9dbb0b5e5b1e?q=80&w=800&auto=format&fit=crop",
      especificaciones: ["API 6A", "R, RX, BX", language === 'es' ? "Acero blando" : "Soft steel", language === 'es' ? "Sellado metal" : "Metal sealing"],
      aplicaciones: ["Wellhead", "Christmas tree", "Offshore", language === 'es' ? "Gas natural" : "Natural gas"]
    },
    {
      tipo: "O-Rings",
      descripcion: language === 'es' ? "Sellos toroidales para aplicaciones dinámicas" : "Toroidal seals for dynamic applications",
      imagen: "https://images.unsplash.com/photo-1581092160562-40aa08e78837?q=80&w=800&auto=format&fit=crop",
      especificaciones: ["NBR, FKM, EPDM", language === 'es' ? "Múltiples durezas" : "Multiple hardness", "AS568, DIN 3771", language === 'es' ? "Alimentario" : "Food grade"],
      aplicaciones: [language === 'es' ? "Hidráulica" : "Hydraulics", language === 'es' ? "Neumática" : "Pneumatics", language === 'es' ? "Equipos rotativos" : "Rotating equipment", language === 'es' ? "Instrumentación" : "Instrumentation"]
    },
    {
      tipo: language === 'es' ? "Juntas Espirometálicas" : "Spiral Wound Gaskets",
      descripcion: language === 'es' ? "Sellado flexible para bridas estándar" : "Flexible sealing for standard flanges",
      imagen: "https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?q=80&w=800&auto=format&fit=crop",
      especificaciones: ["ASME B16.20", "Graphite, PTFE", "316SS", "CG, CGI, CIR"],
      aplicaciones: [language === 'es' ? "Bridas ANSI" : "ANSI flanges", language === 'es' ? "Procesos químicos" : "Chemical processes", language === 'es' ? "Servicios generales" : "General services", language === 'es' ? "Refinación" : "Refining"]
    },
    {
      tipo: language === 'es' ? "Empaques" : "Packing",
      descripcion: language === 'es' ? "Sellado de válvulas y equipos rotativos" : "Valve and rotating equipment sealing",
      imagen: "https://images.unsplash.com/photo-1581092795360-fd1ca04f0952?q=80&w=800&auto=format&fit=crop",
      especificaciones: ["Graphite", "PTFE", language === 'es' ? "Fibra aramida" : "Aramid fiber", "Flexible graphite"],
      aplicaciones: [language === 'es' ? "Válvulas" : "Valves", language === 'es' ? "Bombas" : "Pumps", language === 'es' ? "Compresores" : "Compressors", language === 'es' ? "Equipos rotativos" : "Rotating equipment"]
    },
    {
      tipo: language === 'es' ? "Sellos Especiales" : "Special Seals",
      descripcion: language === 'es' ? "Diseños especiales para aplicaciones críticas" : "Special designs for critical applications",
      imagen: "https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?q=80&w=800&auto=format&fit=crop",
      especificaciones: [language === 'es' ? "Diseño especial" : "Special design", language === 'es' ? "Múltiples materiales" : "Multiple materials", language === 'es' ? "Customizados" : "Customized", language === 'es' ? "Ingeniería" : "Engineering"],
      aplicaciones: [language === 'es' ? "Condiciones extremas" : "Extreme conditions", language === 'es' ? "Fluidos especiales" : "Special fluids", language === 'es' ? "Equipos únicos" : "Unique equipment", language === 'es' ? "Proyectos especiales" : "Special projects"]
    }
  ];
    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden" style={{background: 'linear-gradient(135deg, rgb(10, 10, 10) 0%, rgb(26, 26, 26) 25%, rgb(42, 42, 42) 75%, rgb(15, 15, 15) 100%)'}}>
        ...
        <div className="absolute inset-0 z-0 opacity-[0.08]">
          <img src="https://images.unsplash.com/photo-1581092451866-a33e4b30a2e6?q=80&w=2070&auto=format&fit=crop" alt="Industrial Background" className="w-full h-full object-cover" style={{filter: 'grayscale(100%) contrast(1.2)', mixBlendMode: 'overlay'}} />
        </div>
        ...
        <div className="relative z-20 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center py-16 sm:py-20">
          <div className="mb-6 sm:mb-8 animate-fade-in-up">
            <div className="inline-flex items-center bg-white/5 backdrop-blur-sm border border-white/10 rounded-full px-4 sm:px-6 py-2 mb-6 sm:mb-8">
              ...
              <span className="text-white/80 text-xs sm:text-xs sm:text-sm font-medium tracking-wider uppercase" style={{fontFamily: 'Inter, system-ui, sans-serif'}}>{language === 'es' ? 'SELLADO INDUSTRIAL DE ALTA PERFORMANCE' : 'HIGH PERFORMANCE INDUSTRIAL SEALING'}</span>
            </div>
          </div>

          <div className="mb-6 sm:mb-8 lg:mb-12 animate-fade-in-up-delay-1">
            <h1 className="text-2xl sm:text-3xl lg:text-5xl xl:text-4xl sm:text-5xl lg:text-6xl font-light text-white mb-3 sm:mb-4 lg:mb-6" style={{fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 200, letterSpacing: '0.01em', lineHeight: '1.1'}}>{language === 'es' ? 'JUNTAS Y' : 'GASKETS &'}</h1>
            
            <div className="flex items-center justify-center mb-4 sm:mb-6">
              <div className="h-px w-8 sm:w-12 lg:w-16 bg-gradient-to-r from-transparent to-corporate-red mr-2 sm:mr-4"></div>
              <h2 className="text-2xl sm:text-3xl lg:text-5xl xl:text-4xl sm:text-5xl lg:text-6xl font-bold" style={{fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 800, background: 'linear-gradient(135deg, rgb(255, 0, 0) 0%, rgb(255, 51, 51) 50%, rgb(255, 102, 102) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', letterSpacing: '-0.02em', lineHeight: '1.1'}}>{language === 'es' ? 'SELLOS' : 'SEALS'}</h2>
          ...
          <p className="text-sm sm:text-base lg:text-sm sm:text-base md:text-lg text-white/70 mb-8 sm:mb-12 lg:mb-16 max-w-2xl lg:max-w-4xl mx-auto leading-relaxed animate-fade-in-up-delay-2 px-4" style={{fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 300, letterSpacing: '0.01em'}}>
            {language === 'es' ? 'Juntas y sellos de alta performance para sellado crítico en instalaciones industriales y offshore.' : 'High performance gaskets and seals for critical sealing in industrial and offshore facilities.'}
          </p>
      ...
      <section className="py-4 sm:py-6 bg-gray-50 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center text-xs sm:text-xs sm:text-sm text-gray-600 overflow-x-auto whitespace-nowrap">
            ...
            <span className="text-corporate-red font-medium flex-shrink-0">{language === 'es' ? 'Juntas y Sellos' : 'Gaskets & Seals'}</span>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 sm:mb-12 lg:mb-16">
            <h2 className="text-xl sm:text-2xl lg:text-3xl xl:text-2xl sm:text-3xl md:text-4xl font-light text-gray-900 mb-4 sm:mb-6 px-2" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
              {language === 'es' ? 'Tipos de ' : 'Types of '}<span className="font-semibold text-corporate-red">{language === 'es' ? 'Juntas y Sellos' : 'Gaskets & Seals'}</span>
            </h2>
            <p className="text-sm sm:text-sm sm:text-base text-gray-600 max-w-3xl mx-auto leading-relaxed px-4" style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: '300' }}>
              {language === 'es' ? 'Juntas y sellos de alta performance certificados para sellado crítico con trazabilidad completa.' : 'Certified high performance gaskets and seals for critical sealing with full traceability.'}
            </p>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {tiposJuntas.map((junta, index) => (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <ul className="space-y-1">
                      {junta.especificaciones.map((spec, idx) => (
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {junta.aplicaciones.map((aplicacion, idx) => (
                        <li key={idx} className="flex items-center text-[10px] sm:text-xs text-gray-600">
      ...
      <section className="relative py-12 sm:py-16 lg:py-24 bg-corporate-red overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-xl sm:text-2xl lg:text-3xl xl:text-2xl sm:text-3xl md:text-4xl font-light text-white mb-4 sm:mb-6 px-2" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
            {language === 'es' ? '¿Necesitás ' : 'Need '}<span className="font-semibold text-white">{language === 'es' ? 'juntas de alta performance?' : 'high performance gaskets?'}</span>
          </h2>
          <p className="text-sm sm:text-sm sm:text-base text-white/80 max-w-3xl mx-auto leading-relaxed mb-6 sm:mb-8 lg:mb-10 px-4" style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: '300' }}>
            {language === 'es' ? 'Consultanos para selección técnica, disponibilidad de stock y cotización inmediata.' : 'Contact us for technical selection, stock availability and immediate quote.'}
          </p>
